...
public class JsignCLITest {
    ...
    public void testTimestampingAuthenticode() throws Exception {
        ...
        cli.execute("--keystore=target/test-classes/keystores/" + keystore, "--alias=" + alias, "--keypass=" + keypass, "--tsaurl=http://timestamp.sectigo.com/authenticode", "--tsmode=authenticode", "" + targetFile2);
    ...
    public void testTimestampingRFC3161() throws Exception {
        ...
        FileUtils.copyFile(sourceFile, targetFile2);
        cli.execute("--keystore=target/test-classes/keystores/" + keystore, "--alias=" + alias, "--keypass=" + keypass, "--tsaurl=http://timestamp.sectigo.com/rfc3161", "--tsmode=rfc3161", "" + targetFile2);
    ...
    public void testTimestampingWithProxyUnauthenticated() throws Exception {
        ...
        try {
            ...
            cli.execute("--keystore=target/test-classes/keystores/" + keystore, "--alias=" + alias, "--keypass=" + keypass,
                        "--tsaurl=http://timestamp.sectigo.com/rfc3161", "--tsmode=rfc3161", "--tsretries=1", "--tsretrywait=1",
    ...
    public void testExplicitCertificateChainOnlyOnSingleEntryWhenFirst() throws Exception {
        try {
            ...
            assertEquals("exception message", "The certificate chain in target/test-classes/keystores/jsign-test-certificate-full-chain-reversed.spc does not match the chain from the keystore".replace('/', File.separatorChar), e.getMessage());
        }
    }
}
